...
# ✅ Download video in best quality with platform-specific cookie support
def download_video(url):
    ...
    if "facebook.com" in url:
        cookie_path = os.path.join(os.getcwd(), "facebook_cookies.txt")
    elif "instagram.com" in url:
        cookie_path = os.path.join(os.getcwd(), "instagram_cookies.txt")
    else:
        cookie_path = None  # YouTube doesn't need cookies

    try:
        # Pehle video info extract karo without download to get video id + ext
        ydl_opts_info = {
            "quiet": True,
        ...
        safe_name = sanitize_filename(video_id) + "." + ext
        final_path = os.path.join(output_path, safe_name)
